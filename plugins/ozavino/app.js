...
window.addEventListener('DOMContentLoaded', () => {
  ...
  class Engine {
    ...
    async start(forceRebuild = false){
      if(forceRebuild && this.ctx){
  ...
  function updateReadouts(){
    ...
    if(typeof updateAudioChip === 'function') updateAudioChip();
  }
  ...
  function updateAudioChip(){
    const chip = qs('#audioStateChip');
    if(!chip) return;
    if(!engine.ctx) chip.textContent = 'CTX OFF';
    else chip.textContent = `CTX ${engine.ctx.state.toUpperCase()}`;
  }

  async function safeStartAudio(forceRebuild = false){
    try{
      await engine.start(forceRebuild);
      if(engine.ctx && engine.ctx.state === 'suspended'){
        await engine.ctx.resume();
      }
      const ok = !!(engine.ctx && engine.ctx.state === 'running');
      qs('#audioStatus').textContent = ok ? 'AUDIO ON' : 'AUDIO SUSP';
      qs('#audioBtn').textContent = ok ? 'AUDIO READY' : 'RESUME AUDIO';
      updateAudioChip();
      return ok;
    } catch(err){
      console.error(err);
  ...
  qs('#audioBtn').addEventListener('click', () => safeStartAudio(false));
  qs('#rebuildAudioBtn').addEventListener('click', async () => { await safeStartAudio(true); });
