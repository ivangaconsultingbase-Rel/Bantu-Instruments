...
(() => {
  ...
  class ModeledVoice {
    ...
    start(freq, velocity, p, when = this.ctx.currentTime, hold = 0.28){
      ...
      const model = p.sourceModel;
      const bright = p.bright;
      const body = p.body;
      const decay = p.decay;
      const morph = p.morph;
      const spectralAmt = p.spectral;
      const grain = p.grain;
      ...
      if(model === 'string'){
        ...
        noiseGain.gain.setValueAtTime(0.018 + p.noise * 0.06, when);
        ...
        noiseGain.gain.setValueAtTime(0.008 + p.noise * 0.03, when);
        ...
        noiseGain.gain.setValueAtTime(0.024 + p.noise * 0.06, when);
      ...
      if(spectral.gain) spectral.gain.setValueAtTime(-4 + morph * 12, when);

      lowpass.type = 'lowpass';
      const cutoff = Math.max(180, 240 + p.cutoff * 5000 + p.focus * 900 - p.lofi * 500);
      lowpass.frequency.setValueAtTime(cutoff, when);
      lowpass.Q.setValueAtTime(0.5 + p.resonance * 6, when);
      ...
      const release = 0.18 + decay * 1.2 + p.space * 1.0 + age * 0.6;
      const level = velocity * (0.66 + p.drive * 0.16 + (p.stepAccent || 0));
      ...
      const peak = cutoff + p.env * 2400 + (p.stepAccent || 0) * 1200;
      ...
      noise.stop(when + Math.min(0.18, 0.04 + p.noise * 0.1));
  ...
  class Engine {
    constructor(){
      ...
      this.params = {
        sourceModel:'string',
        tone:0.38, body:0.62, decay:0.82, bright:0.34,
    ...
    panic(){
      this.voices.forEach(v => v.stop(this.ctx ? this.ctx.currentTime : 0));
    }
  ...
  class Sequencer {
    ...
    makeStep(i){
      return {active:i % 2 === 0, degree:(i % 7)+1, chord:i % 8 === 7 ? 'seventh':'triad', octave:0, velocity:0.84, probability:1, ratchet:1, micro:0, tie:0, accent:i % 8 === 4};
    }
    loadInitialPattern(){
  ...
  qs('#playBtn').addEventListener('click', async ()=> { await engine.start(); seq.start(); });
  qs('#stopBtn').addEventListener('click', ()=> seq.stop());
  qs('#previewBtn').addEventListener('click', async ()=> { await engine.start(); seq.scheduleStep(seq.selected, engine.ctx.currentTime + 0.01); });
  ...
  document.addEventListener('keydown', async e => {
    ...
    if(e.code === 'Space'){
      e.preventDefault();
      if(!seq.playing){ await engine.start(); seq.start(); }
      else seq.stop();
    }
  });
