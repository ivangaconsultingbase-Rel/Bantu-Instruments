 ...
 * PATCH:
 * - Ajout paramètres par pad: pitch (semitones) + volume (0..1)
...
export class AudioEngine {
  constructor() {
    ...
    // --- PATCH: paramètres par pad (Pad Editor) ---
    // Pitch en demi-tons (semitones), appliqué via detune en cents.
  ...
  playSample(padId, time = 0, velocity = 1) {
    ...
    // --- PATCH: Pitch (semitones -> cents) ---
    const st = this.padPitch[padId] ?? 0;
    source.detune.value = st * 100;

    // --- PATCH: Gain = base * padVolume * velocity ---
