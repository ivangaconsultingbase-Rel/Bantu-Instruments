...
const STORAGE_KEY = 'spectral-coast-6-v5-patterns';
...
function renderStepGrid() {
  ...
  steps.forEach((step, idx) => {
    ...
    card.type = 'button';
    card.className = `step-card ${step.on ? 'is-on' : 'is-off'} ${idx === seq.selectedStep ? 'is-selected' : ''}`;
    if (seq.playing && highlightPage === seq.page && idx === playingIndex) card.classList.add('is-playing');
    card.dataset.index = String(idx);
    card.innerHTML = `
      <div class="step-top"><span>${seq.page}${idx + 1}</span><span>${Math.round(step.velocity * 100)}</span></div>
      <div class="step-main">${stepLabel(step)}</div>
...
function runSequencerTick() {
  ...
  seq.timeoutId = setTimeout(() => {
    seq.stepIndex = (seq.stepIndex + 1) % totalSteps();
    runSequencerTick();
  }, ms);
}
