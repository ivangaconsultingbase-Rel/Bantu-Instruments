(() => {
  ...
  class Voice {
    ...
    start(freq, velocity, params, when = this.ctx.currentTime, holdSeconds = 0.22){
      ...
      const envAttack = 0.003 + (1 - params.focus) * 0.03 + params.space * 0.02;
      const envDecay = 0.14 + params.space * 0.6 + params.grain * 0.4;
      const sustain = 0.2 + params.morph * 0.3 + params.space * 0.18;
      const release = 0.16 + params.space * 0.7 + params.age * 0.8;
      ...
      filter.frequency.exponentialRampToValueAtTime(Math.max(140, cutoff * (0.7 + sustain * 0.35)), when + holdSeconds + Math.max(0.08, release * 0.65));
  ...
  class Sequencer {
    constructor(engine){
      ...
      this.key = 'C'; this.scale = 'Minor'; this.globalOctave = 0; this.gate = 0.48; this.humanize = 0.01; this.swing = 0.08; this.bpm = 112;
    ...
    randomize(){
      ...
      this.steps.forEach((s, i) => { s.active = Math.random() > 0.42; s.degree = 1 + Math.floor(Math.random() * SCALE_MAP[this.scale].length); s.chord = chordNames[Math.floor(Math.random() * chordNames.length)]; s.velocity = 0.45 + Math.random() * 0.5; s.probability = 0.45 + Math.random() * 0.55; s.ratchet = 1 + Math.floor(Math.random() * 3); s.micro = (Math.random()*2 -1) * 0.025; s.accent = i % 8 === 4 ? true : Math.random() > 0.8; });
    }
    shiftPage(dir){
  ...
  qs('#seqKey').value = sequencer.key; qs('#seqScale').value = sequencer.scale; qs('#bpm').value = sequencer.bpm; qs('#octaveShift').value = sequencer.globalOctave; qs('#gateLength').value = sequencer.gate; qs('#humanize').value = sequencer.humanize; qs('#swing').value = sequencer.swing; qs('#presetSelect').value = 'Wooden Keys';
  ...
  function renderStepGrid(){
    ...
    for(let i=start; i<end; i++){
      ...
      btn.innerHTML = `<div class="step-top"><span>${i+1}</span><span>${step.active ? 'ON' : 'OFF'}</span></div><div class="step-note">${stepLabel(step)}</div><div class="step-flags">${step.accent ? '<span class="badge">ACC</span>' : ''}${step.ratchet > 1 ? `<span class="badge">R${step.ratchet}</span>` : ''}${step.tie ? '<span class="badge">TIE</span>' : ''}</div><div class="prob-bar"><i style="width:${step.probability*100}%"></i></div>`;
  ...
  updateReadouts(); renderScaleNotes(); buildKeyboard(); updateStepEditor(); renderStepGrid(); applyPreset('Wooden Keys');
})();
