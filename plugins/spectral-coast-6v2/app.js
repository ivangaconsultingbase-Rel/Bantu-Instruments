...
const els = {
  ...
  jumpToSeqBtn: document.getElementById('jumpToSeqBtn'),
  quickToggleSeqBtn: document.getElementById('quickToggleSeqBtn'),
  quickPlaySeqBtn: document.getElementById('quickPlaySeqBtn'),
...
function applySectionToggles() {
  document.querySelectorAll('.panel-toggle').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (id === 'groovebox') syncSequencerQuickUi();
    });
  });
}


function syncSequencerQuickUi() {
  const panel = document.querySelector('[data-section="groovebox"]');
  const shown = panel && !panel.classList.contains('is-collapsed');
  if (els.quickToggleSeqBtn) {
    els.quickToggleSeqBtn.textContent = shown ? 'SEQ SHOWN' : 'SEQ HIDDEN';
    els.quickToggleSeqBtn.classList.toggle('active', shown);
  }
}

function scrollToSequencer() {
  const panel = document.getElementById('grooveboxPanel');
  if (!panel) return;
  panel.classList.remove('is-collapsed');
  const mainToggle = document.querySelector('.panel-toggle[data-target-section="groovebox"]');
  if (mainToggle) mainToggle.textContent = 'HIDE';
  syncSequencerQuickUi();
...
async function main() {
  ...
  els.quickPlaySeqBtn?.addEventListener('click', async () => { scrollToSequencer(); await startSequencer(); });
  els.jumpToSeqBtn?.addEventListener('click', scrollToSequencer);
  els.quickToggleSeqBtn?.addEventListener('click', () => {
    const panel = document.querySelector('[data-section="groovebox"]');
    if (!panel) return;
    panel.classList.toggle('is-collapsed');
    const mainToggle = document.querySelector('.panel-toggle[data-target-section="groovebox"]');
    if (mainToggle) mainToggle.textContent = panel.classList.contains('is-collapsed') ? 'SHOW' : 'HIDE';
    syncSequencerQuickUi();
    if (!panel.classList.contains('is-collapsed')) panel.scrollIntoView({ behavior: 'smooth', block: 'start' });
  });
