// plugins/cs01/dsp/macro-osc.worklet.js
// Macro OSC mono — 100% JS (no build). Modes:
// 0 = VA Saw (PolyBLEP), 1 = PWM Pulse (PolyBLEP), 2 = SuperSaw, 3 = FM 2-op, 4 = PhaseDist + Fold

class MacroOscProcessor extends AudioWorkletProcessor {
  static get parameterDescriptors() {
    return [
      { name: "mode", defaultValue: 0 },         // 0..4 (we keep integer-ish)
      { name: "freq", defaultValue: 440 },       // Hz
      { name: "gain", defaultValue: 0.8 },       // 0..1
      { name: "pw", defaultValue: 0.5 },         // 0.05..0.95
      { name: "detune", defaultValue: 0.15 },    // 0..1
      { name: "fm", defaultValue: 0.0 },         // 0..1
      { name: "timbre", defaultValue: 0.5 },     // 0..1
    ];
  }
  ...
  process(_inputs, outputs, parameters) {
    ...
    for (let i = 0; i < out.length; i++) {
      const modeRaw = parameters.mode.length > 1 ? parameters.mode[i] : parameters.mode[0];
      const mode = Math.max(0, Math.min(4, modeRaw | 0));

      const freq0 = parameters.freq.length > 1 ? parameters.freq[i] : parameters.freq[0];
      ...
      const f = Math.max(1, Math.min(freq0, sr * 0.45));
      const dt = f / sr;
      ...
      if (mode === 0) {
        ...
        const idx = fm * (2.0 + 10.0 * timbre);
        const carPhase = this.phase + (mod * idx) * dt;
        ...
        const fold = 1 + fm * 6;
        let z = y * fold;
        z = 2 * (z - Math.floor(z + 0.5)); // triangle fold
        y = this.softClip(z);
      }

      y = this.dcBlock(y);
      out[i] = y * Math.max(0, Math.min(1, gain));
    }
